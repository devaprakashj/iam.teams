...
import { Mail, Lock, ArrowRight, Github, Chrome, UserCheck } from 'lucide-react';
...
export default function SignIn() {
  ...
  const handleSubmit = async (e: React.FormEvent) => {
    ...
    try {
      ...
      if (userCredential.user) {
        ...
        const username = userCredential.user.email ? userCredential.user.email.split('@')[0] : '';
  ...
  const handleGoogleLogin = async () => {
    ...
    try {
      ...
      if (result.user) {
        ...
        const { doc, getDoc } = await import('firebase/firestore');
        const { db } = await import('@/lib/firebase');
        const username = result.user.email ? result.user.email.split('@')[0] : '';
        const docRef = doc(db, 'users', username);
        const docSnap = await getDoc(docRef);
  ...
  const handleDemoLogin = () => {
    setEmail('demo@iamfolio.in');
    setPassword('demo12345');
    
    // Persist login state for the demo
    localStorage.setItem('isLoggedIn', 'true');
    ...
    <div className="min-h-screen bg-[#0A0A0F] text-[#E2E8F0] font-mono flex overflow-hidden selection:bg-[#6C3CE1]/30">
        ...
        <div className="relative z-10 max-w-lg">
          ...
          >
            ...
            <h2 className="text-6xl font-black font-syne text-white leading-[0.9] mb-8 italic">
              ...
              <span className="text-[#6C3CE1]">SHOWCASE GENIUS.</span>
            </h2>
            ...
            >
               ...
               <p className="text-[#E2E8F0] text-lg font-medium italic leading-relaxed font-syne">
                 "iamfolio helped me land my dream job at Zoho. It took me less than 5 minutes to set up a profile that recruiters loved."
               </p>
            ...
            <div className="mt-16 grid grid-cols-3 gap-8">
              ...
              <div>
                <p className="text-3xl font-black font-syne text-white italic">4.9/5</p>
                <p className="text-[#64748B] text-[9px] font-black uppercase tracking-[0.2em] mt-1.5">Rating</p>
              </div>
            </div>
          </motion.div>
        </div>
      ...
      <div className="flex-1 flex items-center justify-center p-4 sm:p-6 bg-[#0A0A0F] relative">
        ...
        <div className="absolute inset-0 overflow-hidden pointer-events-none opacity-30">
          <div className="absolute top-[-10%] right-[-10%] w-[50%] h-[50%] bg-[#6C3CE1]/15 blur-[120px] rounded-full"></div>
        ...
        >
          <div className="text-center mb-8">
            <Link href="/" className="inline-block mb-6 lg:hidden">
              <span className="text-5xl font-black text-[#6EE7B7] tracking-tighter italic font-syne">IF</span>
            </Link>
            <h1 className="text-3xl sm:text-4xl font-black font-syne text-white tracking-tight italic uppercase">Welcome back</h1>
            <p className="text-[#64748B] mt-2 font-bold text-xs uppercase tracking-widest leading-loose">Your professional peak starts here.</p>
          </div>
          ...
          <div className="bg-[#111118]/80 backdrop-blur-xl rounded-[28px] border border-[#1E1E2E] p-6 sm:p-10 relative overflow-hidden shadow-2xl">
            ...
            <div className="relative mb-8 text-center">
              ...
              <span className="relative bg-[#111118] px-4 text-[9px] font-black text-[#64748B] uppercase tracking-[0.3em]">OR USE EMAIL</span>
            </div>
            ...
            <form onSubmit={handleSubmit} className="space-y-6">
              ...
              <div className="group">
                <div className="flex justify-between items-center mb-2.5 ml-1">
                  <label className="text-[10px] font-black text-[#64748B] uppercase tracking-[0.2em]">Password</label>
                  <Link href="#" className="text-[9px] font-black text-[#6C3CE1] uppercase tracking-widest hover:underline">Forgot?</Link>
                </div>
                <div className="relative">
                  <div className="absolute left-4 top-1/2 -translate-y-1/2 border-r border-[#1E1E2E] pr-3">
              ...
              >
                {loading ? 'AUTHENTICATING...' : 'ACCESS IDENTITY'} <ArrowRight className="w-5 h-5 group-hover:translate-x-1.5 transition-transform" />
              </motion.button>
            </form>

            <p className="mt-8 text-center text-[10px] text-[#64748B] font-black uppercase tracking-widest">
              NEW HERE? <Link href="/signup" className="text-[#6EE7B7] hover:underline underline-offset-4 decoration-2">CREATE ACCOUNT</Link>
            </p>

            <div className="relative my-8 text-center">
              <div className="absolute inset-0 flex items-center"><div className="w-full border-t border-[#1E1E2E]"></div></div>
              <span className="relative bg-[#111118] px-4 text-[9px] font-black text-[#64748B] uppercase tracking-[0.3em]">QUICK ACCESS</span>
            </div>

            <button 
              onClick={handleDemoLogin}
              className="w-full py-4 bg-[#0A0A0F] border border-[#1E1E2E] rounded-xl font-black text-[10px] uppercase tracking-[0.2em] text-[#E2E8F0] hover:bg-[#111118] hover:border-[#6C3CE1]/30 transition-all active:scale-95 flex items-center justify-center gap-3 group"
            >
              <UserCheck className="w-4 h-4 text-[#6EE7B7] group-hover:scale-110 transition-transform" />
